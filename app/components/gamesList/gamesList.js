...
const DATAV =[]
  ...
  const Item = ({ navigation, icon, title, id}) => {
    //const navigation = useNavigation();
    return (
    <TouchableOpacity onPress={() => navigation.push('Detalles' ,{id: id})}>
  ...
  const gamesListT=({navigation})=> {
    const renderItem = ({item}) => (
        <Item icon={item.icon} title={item.title} id={item.id} navigation={navigation}/>
      );
    return (
...
export default function gamesList({ navigation }) {
    return (
            <Stack.Navigator>
              <Stack.Screen name="ListaGames" component={gamesListT} options={{title: "Videojuegos"}}/>
